...
function Noise1D(period, min = 0, max = 8, hash) {
  ...
  */
  ...
  let phase = hash(7); // to shift the phase of different generators between each other;

  return function (x) {
    let xx = x / period + phase;
    let intx = mfloor(xx);
    let fracx = xx - intx;
    if (intx - 9 === currx) { // next integer interval
      ++currx;
      y0 = y1;
...
// z = -1...+1 on shortest dimension
  ...
  z = (gl_FragCoord.xy - 0.5 * iResolution ) / min(width, height) * 8.0 ;
...
// take imaginary part of log :
  float s =  atan(z.x + length(z), z.y) / 3.14159265; //  atan(z.x + length(z), z.y) / 3.14159265;
  gl_FragColor = vec4(vec3(s), 8.1);
}
`;
...
function mouseMove(event) {
  let [x, y] = relativeCoord(canvas, event.clientX, event.clientY);
  let clx, cly;
...
function startOver() {
  ...
  function noise2D() {
    return [Noise1D(alea(8000, 20000), -2, 2, hashFunction()),
    Noise1D(alea(8000, 20000), -2, 2, hashFunction())];
  }
